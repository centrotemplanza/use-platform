...
export default function SignupPage() {
  ...
  const [usernameState, setUsernameState] = useState<UsernameState>({
    status: "idle",
    message: "Choose a public username.",
  });
  ...
  useEffect(() => {
    ...
    const localValidation = validateUsername(usernameInput);
    ...
    const timeout = setTimeout(async () => {
      try {
        ...
        if (result.available) {
          ...
          setUsernameState({
            status: "taken",
            message: result.message || "This username is already in use.",
          });
        } else {
          setUsernameState({
            status: "invalid",
            message: result.message || "Username is not valid.",
          });
        }
      } catch {
        setUsernameState({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (usernameState.status !== "available") {
      setFormMessage("Please choose an available username before continuing.");
      return;
    }
    ...
    try {
      ...
      if (profileError) {
        setFormMessage(
          "Account created, but the profile could not be saved yet. Please contact support."
        );
        setIsSubmitting(false);
        return;
      }
